		...
		if($status2){
			foreach($status2 as$k=>$v){
				...
				if($t<=0){
					...
					$goods = pdo_fetch("select title,automatic,is_alert,init_money from".tablename("weliam_indiana_goodslist")."where id='{$v['goodsid']}' and uniacid='{$_W['uniacid']}'");
					$name =pdo_fetch("select nickname from".tablename("weliam_indiana_period")."where goodsid='{$v['goodsid']}' and uniacid='{$_W['uniacid']}'");
                                        $datam = array(
						"first"=>array( "value"=> "恭喜你！你参与的一元猜已中奖！","color"=>"#173177"),
						"keyword1"=>array('value' => $name['nickname'], "color" => "#4a5077"),
						"keyword2"=>array('value' => $goods['title']."(".$goods['init_money']."元专区)", "color" => "#4a5077"),
