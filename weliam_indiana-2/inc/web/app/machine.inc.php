<?php
		...
		if($op == 'display'){
			...
			foreach($result_machineset as $key => $value){
				$sql_sele_goods = "select id,title,periods,maxperiods,picarr,price,init_money,init_num from".tablename('weliam_indiana_goodslist')."where uniacid = :uniacid and id = :id";
				...
				$result_machineset[$key]['times'] = $result_sele_goods['init_num'];
		...
		if($op == 'create_goods'){
			...
			$data['max_num'] = !empty($_GPC['max_number'])?intval($_GPC['max_number']):11;	//最大购买量(默认11)
			$this->WL_log('machine','data',$data['max_num']);
			if(!is_int($data['max_num']) || $data['max_num'] < 1){
				message('最大购买量默认11个；但是填写数只能是正整数，或者不填写！！',referer(),'error');exit;
			}
			
			$data['machine_num'] = !empty($_GPC['machine_num'])?intval($_GPC['machine_num']):1;//购买数量范围（默认购买范围1）
			if(!is_int($data['machine_num']) || $data['machine_num'] < 1){
				message('购买数量范围默认为1；但是填写数只能是正整数，或者不填写！！',referer(),'error');exit;
			}
		...
		if($op == 'open_machine'){
			...
			if($machine_status == 'on'){
				...
				if($result_tenlent == 1){
					...
					$url_weliam = "http://weixin.weliam.cn/addons/weliam_manage/api.php";
