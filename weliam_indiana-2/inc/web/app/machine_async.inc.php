<?php 
	...
	foreach($result as $key=>$value){
		...
		if($result_period['status'] != 1){
			...
			//$machine = m('machine')->get_Machines(1);			//获取一个机器人
                        $machine = GetOneMachines();//获取一个机器人
	...
	message('购买成功',referer(),'success');
        //获取一个不重复的机器人
        function GetOneMachines(){
            $machine=m('machine')->get_Machines(1);
//            $sql_record = "select goodid from".tablename('weliam_indiana_record')."where uniacid=:uniacid and period_number=:period_number and openid=:openid";
//            $data_record = array(
//                ':uniacid'=>6,
//                ':period_number'=>'20170901338767567204',
//                ':openid'=>$machine[0]['openid']
//            );
            
	    //$result = pdo_fetchcolumn($sql_period,$data_period);
            if($machine[0]['openid']=="machine70681"){
                return $machine;
            }else{
                return GetOneMachines();
            } 
        }
	?>