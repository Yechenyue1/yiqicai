...
if ($op == 'display') {
	...
	$resp = ihttp_request('http://weixin.weliam.cn/addons/weliam_manage/api.php', array('type' => 'user','module' => 'weliam_indiana','website' => $siteid,'domain'=> $domain),null,1);
	$resp = @json_decode($resp['content'], true);
	$ip = $resp['ip'];
	
	$result = ihttp_request('http://weixin.weliam.cn/addons/weliam_manage/api.php', array('type' => 'checkauth','module' => 'weliam_indiana'),null,5);
	$result = @json_decode($result['content'], true);
	
	if (checksubmit()) {
		$data = array('ip' => $_GPC['ip'],'domain' => $_GPC['domain'],'siteid' => $_GPC['siteid'],'code' => $_GPC['code']);
		m('syssetting')->syssetting_save($data,'auth');
		$resp = ihttp_request('http://weixin.weliam.cn/addons/weliam_manage/api.php', array('type' => 'grant','module' => 'weliam_indiana','code' => $data['code']),null,1);
		$resp = @json_decode($resp['content'], true);
		if($resp['errno'] == 1){
...
//	$result = ihttp_request('http://weixin.weliam.cn/addons/weliam_manage/api.php', array('type' => 'checkauth','module' => 'weliam_indiana'),null,5);
	$result = @json_decode($result['content'], true);
	if($result['status'] != 1){
		message('已去除授权，禁用更新！',web_url('auth/display'),'warning');
	}
