	...
	if($op=='submit'){
		...
		if(pdo_insert('weliam_indiana_record',$data))
		{
			...
			message('提交失败！');
                ...
                $getcode =$_GPC['getcode'];//xiugai
		$period_number=$_GPC['periodnumber'];
		$titlelist = m('goods')->getListByPeriod_number($period_number);
		$title = $_GPC['title']."<span style='color: #FDCD21;'>(".$titlelist['init_money']."元专区)</span>";
		...
		if($thisperiod['status']!=1){
			...
			if($_GPC['type']=='remove'){
				...
				if(!empty($myCart_this)){
					...
					if(pdo_insert('weliam_indiana_cart',array('num'=>$addnum,'uniacid'=>$_W['uniacid'],'period_number'=>$period_number,'openid'=>$openid,'title'=>$title,'ip'=>$_W['clientip'],'ipaddress'=>$arr['province'].$arr['city'],'getcode'=>$getcode))){
		...
		foreach($myCart as $key=>$value){
			...
			$money +=$value['num']*$goods['init_money'];
                        $myCart[$key]['num_min'] = $goods['num_min'];//最小值
                        $myCart[$key]['num_max'] = $goods['num_max'];//最大值
                        $myCart[$key]['get_code'] = $value['getcode'];//用户输入的数值
		}
