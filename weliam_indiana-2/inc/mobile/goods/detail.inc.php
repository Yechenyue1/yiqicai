<?php
	...
	if($op == 'goods'){
                $goods = getgoodslist($_W['uniacid']);
                //echo '<pre>';var_dump($goods);echo '<pre/>';die();
		//获取商品信息
		...
		$period['init_money'] = $detail_goodslist['init_money'];
		$period['nowtime'] = time();
		...
		if($mycodes){
			//检测个人夺宝码
			$myallcodes = $mycodes['get_code'];//修改
	    ...
	    unset($adv);
            //查询参与信息
            $uniacid =$_W['uniacid'];
            $info = customer($period['period_number'],$uniacid,$period['status']);
//            var_dump($info);die();
	...
	if($op == 'customer'){
		...
		$pagenum = !empty($_GPC['pagenum'])?$_GPC['pagenum']:11;
        ...
        function customer($number,$uniacid,$status){
		//加载商品消费记录
		$period_number = $number;
                $Wuniacid = $uniacid;
		
		/*******检索消费者数据******/
		$sql_customer = "select * from".tablename('weliam_indiana_consumerecord')." where uniacid = :uniacid and period_number = :period_number order by createtime asc ";
		$data_customer = array(
			':uniacid' => $Wuniacid,
			':period_number' => $period_number
		);
...
//			$customer_list[$key]['num'] = $value['num']/$init_money['init_money'];		//将购买金额换算成购买次数
			...
			$customer_list[$key]['createtime'] = date("Y-m-d H:i:s",$value['createtime']);
                ...
                //判断是否已经开奖
                if($status!=1){
                    $records = pdo_fetchall("SELECT * FROM ".tablename('weliam_indiana_record')." WHERE uniacid = '{$Wuniacid}' and period_number = '{$period_number}' and status<>0 order by createtime asc");
                    foreach($records as $key => $value){
                        $customer_list[$key]['get_code'] = $value['get_code'];
                    }
                }
		return $customer_list;
//		die(json_encode(array('status'=>1,'result'=>$customer_list)));exit;		//json返回信息
	...
	$sql_allperiod = "select scale,period_number,canyurenshu,shengyu_codes,zong_codes,periods,goodsid from".tablename('weliam_indiana_period')."where uniacid = :uniacid and status = :status order by ".$rank." limit 0,8";
