<?php
	...
	if ($operation == 'list') {
			...
			if($period){
				foreach ($period as $key => $value) {
					...
					$period[$key]['picarr'] = $goods['picarr'];
                                        $period[$key]['money'] = $goods['init_money'];
				}
		}
		...
		$goods = pdo_fetch("SELECT title,picarr,automatic,init_money FROM " . tablename('weliam_indiana_goodslist') . " WHERE uniacid = '{$_W['uniacid']}' and id ='{$period['goodsid']}'");
		$jinge_num = unserialize($goods['automatic']); // xiugai
                $jinge_num = $jinge_num['num'];
                if(empty($_GPC['aid'])){
