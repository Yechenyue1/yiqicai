<?php 
	...
	$goods = pdo_fetch("select title,automatic,init_money from".tablename('weliam_indiana_goodslist')."where id=:id",array(':id'=>$period['goodsid']));
	...
	if($automatic['select'] == 2){//积分
		...
		if($re == 1){
			...
			$data  = array(
				"first"=>array( "value"=> "你的奖品已经到账。","color"=>"#173177"),
		...
		if($re > 0){
                        ...
                        $mess = '中奖金额:'.$num;
			m('credit')->updateCredit2($period['openid'],$uniacid,$num,$mess);
			$data  = array(
				"first"=>array( "value"=> "你的奖品已经到账。","color"=>"#173177"),
				"keyword1"=>array( "value"=> $goods['title']."(".$goods['init_money']."元专区)","color"=>"#173177"),
				"keyword2"=>array( "value"=> $num."元(已到余额)","color"=>"#173177"),
				"keyword3"=>array( "value"=> $showtime,"color"=>"#173177"),
				"remark"=>array('value' => "\r请到个人中心查看！", "color" => "#4a5077"),
			);
			m('common')->sendTplNotice($period['openid'],$tpl_id_short,$data,$url2,'');
			m('log')->WL_log('automatic','自动发货情况','【第'.$period['periods'].'期商品'.$goods['title'].'】自动发货成功',$uniacid);
		}else{
