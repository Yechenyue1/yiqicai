	...
	foreach($result as $key=>$value){
		...
		if($result_period['status'] != 1){
			...
			//$machine = m('machine')->get_Machines(1);			//获取一个机器人
                        $machine = GetOneMachines($_W['uniacid'],$value['period_number']);//获取一个机器人
...
exit('false');
function GetOneMachines($uniacid,$period_number){
        $machine=m('machine')->get_Machines(1);
        $sql_record = "select id from".tablename('weliam_indiana_record')."where uniacid=:uniacid and period_number=:period_number and openid='".$machine[0]['openid']."';";
        $data_record = array(
            ':uniacid'=>$uniacid,
            ':period_number'=>$period_number,
        );
        $result = pdo_fetch($sql_record,$data_record);
        if(!$result){
           return $machine;
        }else{
           return GetOneMachines($uniacid,$period_number);
       } 
}