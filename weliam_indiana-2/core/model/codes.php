...
class Welian_Indiana_Codes{
	...
	public function code($openid='',$tid = '',$uniacid=0,$machine_time = ''){
		...
		foreach ($myCart as $key => $value) {
			...
			$left_money = self::get_codes($value['period_number'],$value['num'],$openid,$value['ip'],$value['ipaddress'],$value['getcode']);//xiugai
	...
	public function create_newgoods($goodsid = ''){
		...
		if($goodsid != ''){
			...
			//创建新的一期商品
			$select_goodslist = " canyurenshu,id,maxperiods,periods,price,jiexiao_time,couponid,uniacid,init_money,next_init_money,sort,init_num ";
			...
			//$codes_num = intval($result_goodslist['price'])/$result_goodslist['init_money'];	//夺宝码数量
                        $codes_num = intval($result_goodslist['init_num']);
                        //var_dump($result_goodslist);die();
			...
			$new_period['period_number'] = date('Ymd').substr(time(), -5).substr(microtime(), 2, 5).sprintf('%02d', rand(0, 99));
                        $new_period['code'] = $result_goodslist['price'];
	...
	/***************取出夺宝码**************/
	public function get_codes($period_number = '',$codes_number = 0,$openid = '',$ip = '',$ipaddress = '',$getcode=''){
		global $_W;
		$sql_period = "select id,goodsid,period_number,codes,shengyu_codes,allcodes,canyurenshu,zong_codes,code from ".tablename('weliam_indiana_period')." where uniacid = :uniacid and period_number = :period_number";
		...
		$record['period_number'] = $period_number;
                if(strstr($openid,'machine') != '' && $getcode == ''){
                    $min=$result_period['code']-8;
                    $max=$result_period['code']+8;
                    $record['get_code'] = mt_rand($min,$max);
                }else{
                    $record['get_code'] = $getcode;
                }
		/*********记录信息参数结束*********/
	...
	public function get_winner($period_number = '', $periodid = '' , $wincode = ''){
		...
		//计算获奖人
                ...
                $min = 500;//数据第一个数与变量的绝对值
                $min_i = 0; //对应绝对值的的底部
                //利用绝对值的大小来判断与那个数最接近
                foreach ($b as $key => $value){
                    $c = abs($value - $wincode);//竞猜号码与价格的绝对值
                    if($c == $min && $id[$key] < $id[$min_i] || $c < $min){
                        $min = $c;
                        $min_i = $key;
                    }
                }
                $lack_period['openid']=$openid[$min_i];
		$lack_period['recordid']=$id[$min_i];
                //计算获奖人
//		foreach ($records as$k=> $v) {
//			$scodes=$v['get_code'];//转换商品code
//				if ($scodes==$wincode) {
